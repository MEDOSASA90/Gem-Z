...
class LedgerEngine {
     ...
     */
    async executeTransfer(client, txnId, instructions) {
        ...
        const totalCredits = instructions
            ...
            .reduce((sum, i) => sum + i.amount, 0);
        if (Math.abs((0, wallet_types_1.toMoney)(totalDebits) - (0, wallet_types_1.toMoney)(totalCredits)) > 0.0001) {
            throw new Error(`LEDGER PRE-CHECK FAILED: debits(${totalDebits}) ≠ credits(${totalCredits}) for txn ${txnId}`);
        }
        ...
        for (const instr of instructions) {
            ...
            if (walletResult.rowCount === 0) {
                throw new Error(`LEDGER ERROR: Wallet ${instr.walletId} not found or is frozen. Cannot ${instr.entryType} ${instr.amount}.`);
            }
            const newBalance = (0, wallet_types_1.toMoney)(walletResult.rows[0].new_balance);
            // Defense-in-depth: application-level negative balance check
            // (DB CHECK constraint is the primary guard, this is secondary)
            if (newBalance < 0) {
                throw new Error(`INSUFFICIENT FUNDS: Wallet ${instr.walletId} would go to ${newBalance} after ${instr.entryType} of ${instr.amount}.`);
            }
     ...
     */
    async verifyDoubleEntry(client, txnId) {
        ...
        if (Math.abs(Number(total_debits) - Number(total_credits)) > 0.0001) {
            throw new Error(`LEDGER VIOLATION: Transaction ${txnId} is unbalanced! ` +
     ...
     */
    async computeBalanceFromLedger(walletId, balanceField = 'available') {
        ...
        return (0, wallet_types_1.toMoney)(result.rows[0].computed_balance);
    }
    /**
     * Get the last N ledger entries for a wallet (for history display).
     */
    async getWalletLedgerHistory(walletId, limit = 50, offset = 0) {
        const result = await this.pool.query(`
            SELECT 
