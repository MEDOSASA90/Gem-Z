...
class TransactionService {
    ...
    async processPayment(request) {
        ...
        try {
            ...
            const payerWallet = walletsResult.rows.find((w) => w.owner_type === 'user' && w.owner_id === request.payerUserId);
            const receiverWallet = walletsResult.rows.find((w) => w.owner_type === request.receiverOwnerType && w.owner_id === request.receiverOwnerId);
            const platformWallet = walletsResult.rows.find((w) => w.owner_type === 'platform');
            ...
            // 3. Validate balance
            if ((0, wallet_types_1.toMoney)(payerWallet.available_bal) < request.amount) {
                throw new Error(`Insufficient funds. Available: ${payerWallet.available_bal}, Required: ${request.amount}`);
            }
            // 4. Calculate splits
            const platformFee = (0, wallet_types_1.toMoney)((request.amount * request.platformFeePct) / 100);
            const receiverNet = (0, wallet_types_1.toMoney)(request.amount - platformFee);
            // Sanity check
            if (Math.abs((0, wallet_types_1.toMoney)(platformFee + receiverNet) - request.amount) > 0.0001) {
            ...
            const referenceNo = (0, wallet_types_1.generateRefNo)('TXN');
            await client.query(`
    ...
    async processTopUp(request) {
        ...
        try {
            ...
            if (request.gatewayRef) {
                const existingByRef = await client.query(`SELECT id, reference_no FROM transactions WHERE gateway_ref = $1 AND status = 'completed'`, [request.gatewayRef]);
            ...
            const walletsResult = await client.query(`
                ...
                FOR UPDATE
            `, [request.userId]);
            const userWallet = walletsResult.rows.find((w) => w.owner_type === 'user' && w.owner_id === request.userId);
            const platformWallet = walletsResult.rows.find((w) => w.owner_type === 'platform');
            if (!userWallet) {
                // Auto-create user wallet
                const newWallet = await client.query(`INSERT INTO wallets (owner_type, owner_id, currency) VALUES ('user', $1, 'EGP') RETURNING *`, [request.userId]);
                walletsResult.rows.push(newWallet.rows[0]);
            }
            const targetWallet = userWallet || walletsResult.rows.find((w) => w.owner_type === 'user');
            ...
            // Create transaction
            const txnId = (0, uuid_1.v4)();
            const referenceNo = (0, wallet_types_1.generateRefNo)('TOP');
            await client.query(`
                INSERT INTO transactions
                    (id, reference_no, idempotency_key, txn_type, status, total_amount, currency,
    ...
    async processRefund(request) {
        ...
        try {
            ...
            const originalTxn = await client.query(`SELECT * FROM transactions WHERE id = $1 AND status = 'completed'`, [request.originalTxnId]);
            ...
            const totalRefunded = (0, wallet_types_1.toMoney)(existingRefunds.rows[0].total_refunded);
            if (totalRefunded + request.amount > Number(original.total_amount)) {
                throw new Error(`Total refunds (${totalRefunded} + ${request.amount}) would exceed ` +
                    `original amount (${original.total_amount}).`);
            }
            // 3. Load original ledger entries to determine which wallets were involved
            const originalEntries = await client.query(`SELECT * FROM ledger_entries WHERE txn_id = $1 ORDER BY id`, [request.originalTxnId]);
            // 4. Lock all involved wallets
            const walletIds = [...new Set(originalEntries.rows.map((e) => e.wallet_id))];
            await client.query(`SELECT id FROM wallets WHERE id = ANY($1) ORDER BY id FOR UPDATE`, [walletIds]);
            // 5. Calculate proportional refund
            const refundRatio = request.amount / Number(original.total_amount);
            // 6. Create refund transaction
            const txnId = (0, uuid_1.v4)();
            const referenceNo = (0, wallet_types_1.generateRefNo)('RFD');
            await client.query(`
                INSERT INTO transactions
            ...
            const refundInstructions = originalEntries.rows.map((entry) => ({
                walletId: entry.wallet_id,
                entryType: entry.entry_type === 'debit' ? 'credit' : 'debit',
                amount: (0, wallet_types_1.toMoney)(Number(entry.amount) * refundRatio),
            ...
            if (!request.isPartial && totalRefunded + request.amount >= Number(original.total_amount)) {
                await client.query(`UPDATE transactions SET status = 'reversed' WHERE id = $1`, [request.originalTxnId]);
            }
            await client.query('COMMIT');
            return { txnId, referenceNo };
        }
     ...
     */
    async processP2PTransfer(request) {
        const client = await this.pool.connect();
        try {
            await client.query('BEGIN');
            if (request.idempotencyKey) {
            ...
            const senderWallet = walletsResult.rows.find((w) => w.owner_id === request.senderUserId);
            const recipientWallet = walletsResult.rows.find((w) => w.owner_id === request.recipientUserId);
            ...
            if ((0, wallet_types_1.toMoney)(senderWallet.available_bal) < request.amount) {
                throw new Error(`Insufficient funds. Available: ${senderWallet.available_bal}`);
            }
            const txnId = (0, uuid_1.v4)();
            const referenceNo = (0, wallet_types_1.generateRefNo)('P2P');
            await client.query(`
                INSERT INTO transactions
                    (id, reference_no, idempotency_key, txn_type, status, total_amount, currency,
     ...
     */
    async processCoinsRedemption(userId, coinsToRedeem, egpValue, idempotencyKey) {
        const client = await this.pool.connect();
        try {
            await client.query('BEGIN');
            if (idempotencyKey) {
            ...
            const profileResult = await client.query(`SELECT total_points FROM trainee_profiles WHERE user_id = $1 FOR UPDATE`, [userId]);
            ...
            const walletsResult = await client.query(`
                ...
                FOR UPDATE
            `, [userId]);
            const userWallet = walletsResult.rows.find((w) => w.owner_type === 'user' && w.owner_id === userId);
            const platformWallet = walletsResult.rows.find((w) => w.owner_type === 'platform');
            if (!userWallet || !platformWallet) {
                throw new Error('Required wallets not found.');
            }
            // Deduct coins
            await client.query(`UPDATE trainee_profiles SET total_points = total_points - $1 WHERE user_id = $2`, [coinsToRedeem, userId]);
            // Create transaction
            const txnId = (0, uuid_1.v4)();
            const referenceNo = (0, wallet_types_1.generateRefNo)('CRD');
            await client.query(`
                INSERT INTO transactions
                    (id, reference_no, idempotency_key, txn_type, status, total_amount, currency,
     ...
     */
    async processAdjustment(walletId, amount, direction, reason, adminUserId, ipAddress) {
        const client = await this.pool.connect();
        try {
            await client.query('BEGIN');
            // Lock wallets
            ...
            `, [walletId]);
            const targetWallet = walletsResult.rows.find((w) => w.id === walletId);
            const platformWallet = walletsResult.rows.find((w) => w.owner_type === 'platform');
            if (!targetWallet || !platformWallet) {
                throw new Error('Wallet not found.');
            }
            const txnId = (0, uuid_1.v4)();
            const referenceNo = (0, wallet_types_1.generateRefNo)('ADJ');
            await client.query(`
                INSERT INTO transactions
                    (id, reference_no, txn_type, status, total_amount, currency,
            ...
            const instructions = direction === 'credit'
                ? [
    ...
    // ─── Helpers ─────────────────────────────────────────────
    async checkIdempotency(client, key) {
        const existing = await client.query(`SELECT t.id, t.reference_no 
             FROM transactions t 
             WHERE t.idempotency_key = $1 AND t.status = 'completed'`, [key]);
    ...
    async insertAuditLog(client, walletId, action, actorUserId, actorType, newValues, ipAddress, userAgent) {
        await client.query(`
