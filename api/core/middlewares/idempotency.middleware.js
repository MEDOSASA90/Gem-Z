...
function idempotencyMiddleware(req, res, next) {
    ...
    db_1.db.query(`SELECT response_code, response_body FROM idempotency_keys 
         WHERE key = $1 AND expires_at > NOW()`, [key]).then(cached => {
        ...
        res.json = function (body) {
            ...
            if (userId && res.statusCode < 500) {
                db_1.db.query(`INSERT INTO idempotency_keys (key, txn_id, user_id, response_code, response_body)
                     VALUES ($1, $2, $3, $4, $5)
                     ON CONFLICT (key) DO NOTHING`, [key, body?.txnId || body?.data?.txnId || null, userId, res.statusCode, body]).catch((err) => {
