import { Pool } from 'pg';
import {
    Wallet, WalletOwnerType, Currency, WalletHistoryFilters,
    WalletSummary, toMoney
} from './wallet.types';
...
export class WalletService {
    ...
    async getOrCreateWallet(
        ...
        const existing = await this.pool.query(
            `SELECT * FROM wallets WHERE owner_type = $1 AND owner_id = $2 AND currency = $3`,
     ...
     */
    async getPlatformWallet(currency: Currency = 'EGP'): Promise<Wallet> {
        const result = await this.pool.query(
            `SELECT * FROM wallets WHERE owner_type = 'platform' AND currency = $1 LIMIT 1`,
            [currency]
        );
    ...
    async getWalletSummary(userId: string): Promise<WalletSummary> {
        ...
        return {
            wallet,
            todayTopups: toMoney(topupResult.rows[0].total),
            todayWithdrawals: toMoney(withdrawResult.rows[0].total),
     ...
     */
    async getWalletHistory(filters: WalletHistoryFilters) {
        const conditions: string[] = ['le.wallet_id = $1'];
        const params: any[] = [filters.walletId];
        let paramIdx = 2;
     ...
     */
    async checkTopUpLimit(userId: string, amount: number): Promise<{ allowed: boolean; remaining: number }> {
        const wallet = await this.getOrCreateWallet('user', userId);
        ...
        const todayTotal = toMoney(result.rows[0].today_total);
        const remaining = toMoney(wallet.daily_topup_limit - todayTotal);
     ...
     */
    async checkWithdrawLimit(userId: string, walletId: string, amount: number): Promise<{ allowed: boolean; remaining: number }> {
        ...
        const todayTotal = toMoney(result.rows[0].today_total);
        const remaining = toMoney(wallet.daily_withdraw_limit - todayTotal);
    ...
    private mapWalletRow(row: any): Wallet {
        return {
            ...
            available_bal: toMoney(row.available_bal),
            pending_bal: toMoney(row.pending_bal),
            frozen_bal: toMoney(row.frozen_bal || 0),
            lifetime_earned: toMoney(row.lifetime_earned),
            lifetime_spent: toMoney(row.lifetime_spent || 0),
            daily_topup_limit: toMoney(row.daily_topup_limit || 50000),
            daily_withdraw_limit: toMoney(row.daily_withdraw_limit || 25000),
