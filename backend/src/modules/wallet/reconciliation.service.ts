import { Pool } from 'pg';
import { ReconciliationReport, toMoney } from './wallet.types';
...
export class ReconciliationService {
     ...
     */
    async snapshotAndReconcile(): Promise<ReconciliationReport> {
        ...
        for (const wallet of wallets.rows) {
            ...
            const storedAvailable = toMoney(wallet.available_bal);
            const discrepancy = toMoney(Math.abs(storedAvailable - computedAvailable));
            ...
            if (isReconciled) {
                ...
                console.error(
                    `[RECONCILIATION] DISCREPANCY in wallet ${wallet.id} ` +
        ...
        console.log(
            `[RECONCILIATION] ${today}: ${reconciled}/${wallets.rows.length} wallets reconciled. ` +
    ...
    async getReportByDate(date: string) {
        ...
        const reconciled = snapshots.rows.filter((s: any) => s.is_reconciled).length;
        const discrepancyRows = snapshots.rows.filter((s: any) => !s.is_reconciled);
