...
import { WithdrawalRequest, WithdrawRequest, toMoney, generateRefNo, LedgerInstruction } from './wallet.types';
...
export class WithdrawalService {
    ...
    async requestWithdrawal(request: WithdrawRequest): Promise<WithdrawalRequest> {
        ...
        try {
            ...
            // Balance check
            if (toMoney(wallet.available_bal) < request.amount) {
            ...
            const fee = 0;
            const netAmount = toMoney(request.amount - fee);
            ...
            const txnId = uuidv4();
            const referenceNo = generateRefNo('FRZ');
            ...
            await client.query(
                `UPDATE wallets SET available_bal = available_bal - $1, frozen_bal = frozen_bal + $1 WHERE id = $2`,
                [request.amount, request.walletId]
            );

            // Insert ledger entries for the freeze
            const newAvailable = toMoney(Number(wallet.available_bal) - request.amount);
            const newFrozen = toMoney(Number(wallet.frozen_bal || 0) + request.amount);
            ...
            await client.query(
                `UPDATE withdrawal_requests SET txn_id = $1 WHERE id = $2`,
                [txnId, withdrawalId]
            );
    ...
    async approveWithdrawal(requestId: string, adminUserId: string): Promise<void> {
        ...
        try {
            ...
            await client.query(
                `UPDATE wallets SET frozen_bal = frozen_bal - $1 WHERE id = $2`,
     ...
     */
    async rejectWithdrawal(requestId: string, adminUserId: string, reason: string): Promise<void> {
        const client = await this.pool.connect();
        ...
        try {
            ...
            // Create unfreeze transaction
            const txnId = uuidv4();
            const referenceNo = generateRefNo('UFZ');
    ...
    async cancelWithdrawal(requestId: string, userId: string): Promise<void> {
        ...
        try {
            ...
            `, [txnId, generateRefNo('CXL'), req.amount, userId]);
     ...
     */
    async listWithdrawals(walletId: string, limit = 20, offset = 0) {
