...
export class StoreController {
    ...
    static async addProduct(req: AuthRequest, res: Response) {
        try {
            ...
            const { rows } = await db.query(insertQuery, [storeId, name, description, price, stock, category, discount, images]);
    ...
    static async checkoutCart(req: AuthRequest, res: Response) {
        ...
        try {
            ...
            for (const item of items) {
                ...
                if (product.stock_qty < item.quantity) throw new Error(`Insufficient stock for ${product.name}`);
    ...
    static async listMarketplaceItem(req: AuthRequest, res: Response) {
        try {
    ...
    static async getMarketplaceItems(req: AuthRequest, res: Response) {
        try {
    ...
    static async listWorkoutTemplate(req: AuthRequest, res: Response) {
        try {
            const trainerId = req.user?.userId;
            const { title, description, priceEgp, fileUrl } = req.body;
            if (!title || !priceEgp) return res.status(400).json({ success: false, message: 'Missing template details' });
    ...
    static async mintFitnessNFT(req: AuthRequest, res: Response) {
        try {
