...
export class FinancialEngine {
    ...
    async processSubscriptionPayment(
        ...
        platformFeePct: number, // e.g., 15.00
        description: string
    ): Promise<string> {
      ...
      `, [traineeId, gymId]);
            ...
            const traineeWallet = walletsRes.rows.find((w: any) => w.owner_type === 'user');
            const gymWallet = walletsRes.rows.find((w: any) => w.owner_type === 'gym');
            const platformWallet = walletsRes.rows.find((w: any) => w.owner_type === 'platform');
            ...
            const txnId = uuidv4();
            const refNo = `TXN-SUB-${uuidv4().substring(0, 8).toUpperCase()}`;
      ...
      `, [txnId, refNo, totalAmountPaid, description, traineeId]);
            ...
            await this.insertLedgerEntryAndMutateWallet(
                client, txnId, traineeWallet.id, 'debit', totalAmountPaid, newTraineeBal, 'TRAINEE_AVAILABLE'
            );

            // --- CREDIT Gym (To Pending Balance) ---
            const newGymPendingBal = Number(gymWallet.pending_bal) + gymRevenue;
            await this.insertLedgerEntryAndMutateWallet(
                client, txnId, gymWallet.id, 'credit', gymRevenue, newGymPendingBal, 'GYM_PENDING',
                Number(gymWallet.lifetime_earned) + gymRevenue
            );

            // --- CREDIT Platform (To Available Balance) ---
            const newPlatformBal = Number(platformWallet.available_bal) + platformFee;
            await this.insertLedgerEntryAndMutateWallet(
                client, txnId, platformWallet.id, 'credit', platformFee, newPlatformBal, 'PLATFORM_AVAILABLE',
                Number(platformWallet.lifetime_earned) + platformFee
            );
     ...
     */
    private async insertLedgerEntryAndMutateWallet(
        client: PoolClient,
        txnId: string,
        walletId: string,
        ...
        targetBalance: 'TRAINEE_AVAILABLE' | 'GYM_PENDING' | 'PLATFORM_AVAILABLE',
     ...
     */
    private async verifyDoubleEntryOrThrow(client: PoolClient, txnId: string) {
     ...
     */
    async depositPlatformCredit(userId: string, amount: number, description: string): Promise<string> {
        ...
        try {
            ...
            `, [txnId, `TXN-CRD-${uuidv4().substring(0, 8).toUpperCase()}`, amount, description, userId]);
