...
class ChallengeController {
    static async listChallenges(req, res) {
        try {
            ...
            const topPointsQuery = await db_1.db.query(`
            ...
            const topStreakQuery = await db_1.db.query(`
                SELECT u.full_name as name, tp.streak_days as days 
                FROM trainee_profiles tp
                JOIN users u ON tp.user_id = u.id
                ORDER BY tp.streak_days DESC LIMIT 10
            `);
            const mapLeaderboard = (rows, scoreKey, scoreSuffix = '') => rows.map((r, i) => ({
                rank: i + 1,
                name: r.name,
    ...
    static async createLiveSquadChallenge(req, res) {
        try {
    ...
    static async trackUserHabit(req, res) {
        try {
    ...
    static async getCorporateLeaderboard(req, res) {
        try {
